...
```{r fig4, fig.cap="Radiance 2015", message=FALSE, warning=FALSE}
darknesssareas_raster <- get_NTLraster("NLD", bearer, "VNP46A4", 2015)

ggplot()+
  geom_tile(aes(x, y, colour=value), data=darknesssareas_raster)+
  geom_sf(data=darknessareas_sf, aes(fill=name, alpha=0.1))+
  coord_sf(xlim = c(3.2821, 4.2995), ylim = c(51.1755,51.8033))
```

#### Veerse Meer

```{r fig3, fig.cap="Radiance 2015", message=FALSE, warning=FALSE}
library(terra)
library(sf)

veersemeer <- darknessareas_sf |> 
  filter(name == "Veerse Meer") |> st_as_sf()

veersemeer_raster <- get_NTLraster("NLD", bearer, "VNP46A4", 2015)

veersemeer_raster <- veersemeer_raster |> rast()
veersemeer_map <- mask(veersemeer_raster, veersemeer)
veersemeer_df <- veersemeer_map |> rasterToPoints(spatial = TRUE) |> as.data.frame() |> 
  rename()


ggplot()+
  geom_tile(data=veersemeer_df, aes(x, y, fill=layer))
```
